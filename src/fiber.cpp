...
// 协程结束后，回收内存
Fiber::~Fiber() {
    SYLAR_LOG_INFO(g_logger) << "Fiber::~Fiber() " << m_id;
    --s_fiber_count;
    if (m_stack) {
...
void Fiber::swapIn() {
    // 当前正在运行的协程切换到后台
    SYLAR_LOG_INFO(g_logger) << this->m_id << " swapIN";
    SetThis(this);
    SYLAR_ASSERT(m_state != EXEC);
    m_state = EXEC;
    // 存储当前协程的上下文，恢复this的上下文
    SYLAR_LOG_INFO(g_logger) << "swap to m_ctx";
...
// 结束执行，切换到后台，让出执行权
void Fiber::swapOut() {
    SYLAR_LOG_INFO(g_logger) << this->m_id << " swapOUT";
...
void Fiber::call() {
    ...
    m_state = EXEC;
    SYLAR_LOG_INFO(g_logger) << "Fiber::call() " << getID();
    // 强行将当前协程转换成目标执行协程，恢复m_ctx中的上下文，将当前上下文存入t_threadFiber的m_ctx中
    SYLAR_LOG_INFO(g_logger) << "swap to t_threadFiber->this";
...
void Fiber::MainFunc() {
    ...
    SYLAR_ASSERT(cur);
    try {
        SYLAR_LOG_INFO(g_logger) << "cur->m_cb()";
        cur->m_cb();
        cur->m_cb = nullptr;
        cur->m_state = TERM;
...
void Fiber::CallerMainFunc() {
    ...
    cur.reset();             // 重置为空
    SYLAR_LOG_INFO(g_logger) << "raw_ptr->back()";
    raw_ptr->back();
